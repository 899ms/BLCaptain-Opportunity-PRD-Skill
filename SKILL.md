---
name: opportunity-to-commercial-prd
description: |
...
6. 选择最小方法论组合。使用 `references/methodology-router.md`。
7. 按 G0 到 G8 运行 Gate。使用 `references/workflow-gates.md`。
8. 输出机会评估报告；只有 Go 时继续输出商业化机会 + 工程实施 PRD。使用 `references/commercial-prd-contract.md`。
9. 需要真实 URL、用户导出样本或真实模型配置演练时，运行 `scripts/prepare_real_run.py`，规则见 `references/real-run-playbook.md`。
10. 需要端到端执行时，运行 `scripts/run_opportunity_workflow.py`，规则见 `references/workflow-orchestration.md`。
11. 生成或检查文件时，运行 `scripts/quick_validate.py` 或 `scripts/validate_opportunity_prd.py`。
